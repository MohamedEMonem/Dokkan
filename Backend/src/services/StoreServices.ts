...
export class StoreServices {
  ...
  async updateStore(userId: string, datatoupdate: updateStoreDto) {
    ...
    if (owner?.role !== "StoreOwner" && owner?.role !== "Admin") {
        const error = new Error("User is not authorized to edit this store") as Error & { statusCode?: number };
    ...
    if (!store || store.subdomain !== storeSubdomain) {
        const error = new Error("Store not found or you do not own this subdomain") as Error & { statusCode?: number };
