...
Conventions
- Error handling: All unexpected errors must be forwarded to Express global error handler via `next(err)` with `err.status` when appropriate. See `docs/error-handling.md` and `docs/middleware.md`.
...
Testing
- Use Postman collection at `docs/Dokkan_Auth.postman_collection.json` for auth flows.
