...
export const GoogleLoginButton = ({ role, action }: GoogleLoginButtonProps) => {
  ...
  // We pass the action (and role if signup) in the state parameter
  // so the callback page knows what to do when Google redirects back.
  const statePayload = JSON.stringify({ action, role });
  ...
  const buttonText = action === 'login' ? "تسجيل الدخول باستخدام جوجل" : "إنشاء حساب باستخدام جوجل";
